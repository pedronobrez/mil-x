...
#       -> dist/release/MIL-X-<version>-linux-x86_64.tar.gz
#          dist/release/MIL-X-<version>-linux-arm64.tar.gz
#          dist/release/MIL-X-<version>-windows-x64.zip
#          dist/release/MIL-X-<version>-macos-arm64.dmg
#          dist/release/MIL-X-<version>-macos-x86_64.dmg
...
#
# The application is built with MILX_SHIP_SCIEX=false and the components are added afterwards,
...
linux_archive () { # <runtime identifier> <name in the archive>
  ...
  local src="$ROOT/dist/milx-desktop-$rid"
  local stage; stage="$(mktemp -d)/MIL-X-$VERSION-$arch"
  mkdir -p "$stage"
  cp -R "$src"/. "$stage/"
  chmod +x "$stage/MIL-X"
  carry_sciex "$stage"
  ...
  ( cd "$(dirname "$stage")" && tar -czf "$OUT/MIL-X-$VERSION-$arch.tar.gz" "$(basename "$stage")" )
  rm -rf "$(dirname "$stage")"
  echo "   $OUT/MIL-X-$VERSION-$arch.tar.gz"
}
...
if wanted windows; then
  ...
  SRC="$ROOT/dist/milx-desktop-win-x64"
  STAGE="$(mktemp -d)/MIL-X-$VERSION-windows-x64"
  mkdir -p "$STAGE"
  cp -R "$SRC"/. "$STAGE/"
  carry_sciex "$STAGE"
  carry_paperwork "$STAGE"
  ...
  ( cd "$(dirname "$STAGE")" && zip -q -r "$OUT/MIL-X-$VERSION-windows-x64.zip" "$(basename "$STAGE")" )
  rm -rf "$(dirname "$STAGE")"
  echo "   $OUT/MIL-X-$VERSION-windows-x64.zip"
fi
...
macos_dmg () { # <runtime identifier> <name in the archive>
  ...
  local stage; stage="$(mktemp -d)/MIL-X $VERSION"
  mkdir -p "$stage"
  ditto "$ROOT/dist/MIL-X.app" "$stage/MIL-X.app"
  # changing what is inside a signed bundle invalidates the signature, so it is signed again after
  carry_sciex "$stage/MIL-X.app/Contents/MacOS"
  codesign --force --deep --sign - --timestamp=none "$stage/MIL-X.app" 2>/dev/null
  codesign --verify --deep "$stage/MIL-X.app" || { echo "the bundle lost its signature" >&2; exit 1; }
  ln -s /Applications "$stage/Applications"
  carry_paperwork "$stage"
  readme macos "$stage/README.txt"
  local dmg="$OUT/MIL-X-$VERSION-$arch.dmg"
  rm -f "$dmg"
  hdiutil create -quiet -volname "MIL-X $VERSION" -srcfolder "$stage" -ov -format UDZO "$dmg"
  rm -rf "$(dirname "$stage")"
  echo "   $dmg"
}
...
for f in "$OUT"/MIL-X-"$VERSION"-*.tar.gz "$OUT"/MIL-X-"$VERSION"-*.zip "$OUT"/MIL-X-"$VERSION"-*.dmg; do
  [ -e "$f" ] || continue
  case "$f" in
...
# ---- checksums --------------------------------------------------------------------------------
( cd "$OUT" && shasum -a 256 MIL-X-"$VERSION"-* > SHA256SUMS && cat SHA256SUMS )
