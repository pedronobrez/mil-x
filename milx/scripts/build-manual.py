#!/usr/bin/env python3
"""Builds the MIL-X manual as one HTML file and one PDF.
...
internal links, renders the HTML with pandoc and prints the PDF with Google Chrome.
    ...
    milx/scripts/build-manual.py [--out DIR]     -> dist/MIL-X-manual-<version>.html and .pdf
...
LANGUAGES = {
    "en": {"folder": MANUAL, "sections": {s: s for s in SECTION_ORDER}, "subtitle": "The manual", "tracks": "tracks MS-DIAL 5.5.260817",
           "referenced": "Referenced by", "title": "MIL-X manual"},
    "pt": {"folder": os.path.join(MANUAL, "pt"),
           ...
           "subtitle": "O manual", "tracks": "acompanha o MS-DIAL 5.5.260817", "referenced": "Referenciado por", "title": "Manual do MIL-X"},
}
...
def compile_markdown(pages: list[dict], images_dir: str, lang: str = "en") -> str:
    ...
    out = [f'<div class="cover"><div class="wordmark">MIL-X</div><div class="sub">{words["subtitle"]}</div>'
...
def main() -> int:
    ...
    stem = f"MIL-X-manual-{version()}" if args.lang == "en" else f"MIL-X-manual-{args.lang}-{version()}"
