#!/usr/bin/env python3
"""Drives the installed MIL-X and checks it actually works.
...
MILX_UI_PROBE names one, rather than guessing from pixels.
...
Two modes. They drive different projects, so run them as two passes rather than together:
    ...
    milx/scripts/smoke-ui.py --process ~/…/raw-folder --library ~/…/library.msp
...
def running() -> bool:
    return bool(subprocess.run(["pgrep", "-f", "MIL-X.app/Contents/MacOS"],
...
def quit_running() -> None:
    ...
    for _ in range(10):
        if not running():
            return
        time.sleep(0.5)
    subprocess.run(["pkill", "-f", "MIL-X.app/Contents/MacOS"], capture_output=True)
    for _ in range(10):
        if not running():
            return
        time.sleep(0.5)
    subprocess.run(["pkill", "-9", "-f", "MIL-X.app/Contents/MacOS"], capture_output=True)
    time.sleep(1)
...
def write_project(folder: str, raw_folder: str, method: str | None, library: str | None, limit: int) -> tuple[str, int]:
    """Writes a .milx project the way the New project wizard would, from the raw files in a folder."""
    ...
    path = os.path.join(folder, "smoke-cycle.milx")
...
def main() -> int:
    ...
    parser.add_argument("--bundle", default="/Applications/MIL-X.app",
    ...
    if not os.path.isdir(args.bundle):
        say(f"FAIL: no application at {args.bundle}; build and install it first "
            f"(milx/scripts/make-app-bundle.sh)")
        return 1
    ...
    work = tempfile.mkdtemp(prefix="milx-smoke-")
    ...
    launch = ["open", "-a", args.bundle, "--env", f"MILX_UI_PROBE={probe}",
              "--env", f"MILX_SETTINGS_DIR={settings}",
    ...
    try:
        ...
        if args.process:
            ...
            check(os.path.exists(os.path.join(out_folder, "milx_method.txt")), "the method was written beside the results")
            ...
            check(bool(saved.get("MdprojectPath")), "the MIL-X project now points at the MS-DIAL project")
            shot("processed")
            ...
            if can_click:
                ...
                check(state["help"]["page"] == "index" and state["help"]["title"].startswith("Manual do MIL-X"),
                ...
                check(state["help"]["title"] == "MIL-X manual", "English brings the English page back")
        ...
        if can_click:
            ...
            if args.project:
                ...
                mx, my, mw, mh = window_frame_named("MIL-X")
                grip = (ix + iw / 2, iy + 12)
        ...
        alive = subprocess.run(["pgrep", "-f", "MIL-X.app/Contents/MacOS"], capture_output=True)
