...
It lives in `~/Library/Caches/MIL-X/spectra` on macOS, `$XDG_CACHE_HOME/MIL-X/spectra` on
Linux and under `LocalAppData\MIL-X\spectra` on Windows; `MILX_CACHE` moves it.
...
conversion cache folder set in [[settings]] (`MILX_MZML_CACHE` does the same). It is reused
while it is newer than the source; `MILX_MSCONVERT_FORCE=1` reconverts. These files are full
...
`settings.json` in the application-data folder — `~/.config/MIL-X` on macOS and Linux,
`%APPDATA%\MIL-X` on Windows, or wherever `MILX_SETTINGS_DIR` points. What it holds is in
