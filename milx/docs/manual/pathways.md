...
| **Beyond BioPAN** | also the steps MIL-X adds to the network for classes a lipidomics run confirms and BioPAN does not cover — Cer ↔ HexCer ↔ LacCer, HexCer ↔ SHexCer, Chol ↔ CE, FA ↔ CAR, MG → FA, PE → PA — marked `extension` in the table; off by default, so the default result is BioPAN's network and nothing else |
...
a class whose only confirmed feature is its standard does not appear. And the **Molecular
species** level follows BioPAN's rule for the reactions that add or remove a chain: PC 34:1 →
LPC 16:0 is drawn only when the fatty acid it releases, FA 18:1, is itself confirmed in the
