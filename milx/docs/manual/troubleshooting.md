...
application and the `milx-cli` launcher force the invariant culture, so it cannot happen
...
centroiding with SCIEX's peak finder (`MILX_WIFF_CENTROID` unset or `sciex`). The
...
`MILX_TRACE=1` prints Avalonia's binding warnings to the console. The log holds the engine's
