...
```bash
bash milx/scripts/setup-macos.sh          # the .NET 8 SDK into ~/.dotnet, no sudo
bash milx/scripts/fetch-sciex-assemblies.sh   # optional: the SCIEX SDK, for native .wiff
...
`milx/Directory.Build.props` and is stamped into the application, the bundle and this
...
`milx/vendor/sciex` together with the license. They are not committed. When the folder is
...
| Suite | What it holds |
| --- | --- |
| `MilX.RawData.Tests` | the mzML reader: encodings, Numpress round trips, the semantics downstream code relies on, the vendor bridge |
| `MilX.Pipeline.Tests` | the curation store, the peak editor, the library search, the caches, the statistics (with equivalence tests between the plain and the fast cross-validation), the synthetic end-to-end run, and real-data tests that run when `MILX_TEST_ALIGNMENT` points at a real result |
| `MilX.Desktop.Tests` | the real views, headless: the review workspace, the statistics workspace, the gestures, the fonts, the probe, the manual in both languages, and the visual regression frames |
| `MilX.Interop.OpenQuant.Tests` | the component CSV and the batch import |
| `MilX.Plugins.SciexWiff.Tests` | the native reader, on a real `.wiff` when `MILX_TEST_WIFF` names one |
...
under `tests/MilX.Desktop.Tests/Baselines/`. The frames travel between machines because the
...
It launches `/Applications/MIL-X.app` through LaunchServices with an isolated settings
...
python3 milx/scripts/build-manual.py --lang pt   # -> milx/dist/MIL-X-manual-pt-<version>.pdf and .html
```
