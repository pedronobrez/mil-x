...
The title reads `MIL-X — <project name>`, with a `•` after it while the project has changes that
...
| Open project… | `⌘O` | an `.milx` or an `.mdproject` |
...
| Save project | `⌘S` | writes the `.milx`; asks for a location the first time |
...
| Item | Shortcut | What it does |
| --- | --- | --- |
| MIL-X manual | `F1` | this manual, at its first page |
| Help for this workspace | `⌘⇧?` | the page for the workspace that is showing |
| Keyboard shortcuts | | [[keyboard-shortcuts]] |
| Troubleshooting | | [[troubleshooting]] |
| About MIL-X | | the version, the MS-DIAL version it tracks, the runtime, the license |
...
The bundle registers `.milx` (and `.odproj`, its name until 1.0), `.mdproject`, `.oqproj`/`.opvproj` and the raw formats (`.mzML`,
