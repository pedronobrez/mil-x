...
incluídos. `MILX_THEME=dark` ou `light` força um para uma sessão sem tocar na configuração.
...
`settings.json` sob `~/.config/MIL-X` no macOS e Linux, `%APPDATA%\MIL-X` no Windows, ou a
pasta que `MILX_SETTINGS_DIR` nomeia. Além das três seções acima, lembra o que a aplicação
aprende com o uso:
