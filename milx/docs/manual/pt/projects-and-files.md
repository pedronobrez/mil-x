...
order: 30
summary: O arquivo de projeto do MIL-X, o projeto MS-DIAL, todo arquivo de uma pasta de resultados e o que cada um guarda, os arquivos de curadoria, as cópias de segurança, e como os caminhos são mantidos.
---

# Projetos e arquivos

## O projeto MIL-X (.milx)
...
Até a 1.0 o mesmo arquivo era `.odproj`. Um daquela época continua abrindo em todo lugar onde um
`.milx` abre, e o próximo **Save** o grava como `.milx` ao lado do arquivo antigo, que fica como
está.
...
ao lado. O MS-DIAL no Windows abre-o; o MIL-X abre-o com **Open project…** ou pelo Finder, e
reconstrói o lote a partir dele quando não há `.milx`. É o único arquivo que leva a biblioteca
...
no Linux. Quando o caminho gravado não existe, o MIL-X procura o conjunto de dados pelo nome ao
...
| `AlignResult-<stamp>_curation.json` | o arquivo lateral do MIL-X: comentários, nomes escolhidos à mão, bandeiras de revisado |
...
`milx_method.txt` quando existe — o bastante para revisar, não o bastante para correr de novo
