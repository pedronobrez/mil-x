...
A aplicação de desktop e o lançador `milx-cli` forçam a cultura invariante, de modo que não
...
está centroidando com o detector de picos da SCIEX (`MILX_WIFF_CENTROID` indefinida ou
...
estar ao lado do `.arf2` que o MIL-X abriu.
...
`MILX_TRACE=1` imprime no console os avisos de binding do Avalonia. O log guarda as mensagens
