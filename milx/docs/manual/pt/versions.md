...
section: Help
order: 61
summary: O que cada versão trouxe, do OpenDIAL 0.1 ao MIL-X 1.0; as notas são atualizadas com toda versão.
---

# Versões

A versão mostrada em **Help ▸ About MIL-X** e na sonda da janela é a carimbada em
...
**O que a renomeação muda para você**

| Até a 0.9 | A partir da 1.0 | O que acontece com o antigo |
| --- | --- | --- |
| `OpenDIAL.app`, `OpenDIAL.exe`, `OpenDIAL` | `MIL-X.app`, `MIL-X.exe`, `MIL-X` | no Windows o instalador substitui o OpenDIAL 0.9 (mesmo código de atualização); no macOS apague o `OpenDIAL.app` você mesmo |
| `.odproj` | `.milx` | continua abrindo, pelo Finder, pelo Explorer e por **Open project…**; o próximo **Save** grava `.milx` ao lado |
| `opendial_method.txt` numa pasta de resultados | `milx_method.txt` | continua sendo lido |
| `~/.config/OpenDIAL`, `%APPDATA%\OpenDIAL` | `~/.config/MIL-X`, `%APPDATA%\MIL-X` | lido uma vez, no primeiro início, enquanto a pasta nova não existe |
| `~/Library/Caches/OpenDIAL` | `~/Library/Caches/MIL-X` | é cache; apague o antigo |
| variáveis `OPENDIAL_*` | `MILX_*` | ainda honradas na 1.0 — veja [[environment-variables]] |
| `%LOCALAPPDATA%\OpenDIAL` | `%LOCALAPPDATA%\MIL-X` | o instalador move a instalação |
| `org.opendial.desktop` | `io.github.pedronobrez.milx` | o macOS pede acesso às suas pastas mais uma vez |
| `opendial/` no repositório | `milx/` | todo caminho de script deste manual mudou junto |

Uma pasta de resultados não mudou: os arquivos do MS-DIAL, o `_tags.xml`, os arquivos de curadoria
e de polaridade são exatamente o que a 0.9 gravava, e a 0.9 continua abrindo-os.

**Novo desde a 0.9.0**

- **`.wiff` nativo de fábrica, em toda plataforma.** As releases carregam os componentes da SCIEX
  que o Apêndice A da licença deles nomeia como redistribuíveis, com essa licença ao lado, em
  `plugins/sciex`. Nada precisa ser buscado ou instalado. Veja [[raw-data-formats]].
- **Um projeto pode ser movido.** Quando o `.mdproject` para o qual um projeto aponta sumiu, os
  resultados são procurados ao lado do projeto, uma pasta acima e nas pastas irmãs, com os
  separadores de qualquer plataforma. Veja [[projects-and-files#O projeto MIL-X (.milx)]].
- **A região de 95 % nos gráficos de escores, nas duas definições.** O padrão é a região χ² que o
  MetaboAnalyst desenha; **F region** é a outra dele, mais larga para uma classe pequena. Veja
  [[statistics-workspace#Principal components]].
- **Duas builds a mais**: Linux em arm64 e macOS em Intel.
- **Um lote pela build do Windows, de ponta a ponta**, numa máquina de verdade, pelo instalador: a
  demo do treinamento, quatro injeções, `.wiff` nativo. Iniciar e funcionar agora são a mesma
  afirmação.

**O que a 1.0 ainda não é**

- **Assinada.** Toda plataforma ainda recebe quem chega com um aviso — "Windows protected your
  PC", "desenvolvedor não identificado". Um certificado de assinatura de código no Windows e uma
  notarização da Apple são o conserto, e nenhum dos dois é uma mudança no programa.
...
Segue o MS-DIAL 5.5.260817. A última versão chamada OpenDIAL.
...
instalador do Windows que registra o `.odproj`. Os downloads da 0.9.0 foram reconstruídos depois da
release para carregar os componentes redistribuíveis da SCIEX, então o `.wiff` também é lido
nativamente a partir deles. Veja [[getting-started]].
...
- A tabela de vias é agora a do próprio BioPAN, transcrita da base arquivada da ferramenta: 97 reações (51 entre classes, 13 sobre os lipídios éter, 3 sobre as bases de esfinganina, 30 entre ácidos graxos) com as suas 249 ligações reação–gene; os doze passos que o MIL-X acrescenta estão marcados e ligados por **Beyond BioPAN**, desligados por padrão. As classes éter separam-se em `O-` e `P-`, as ceramidas e esfingomielinas nas suas formas de esfinganina, como o BioPAN as tem.
...
- A pasta de configurações pode ser movida com `MILX_SETTINGS_DIR`.
