...
The application uses a small, fixed vocabulary. Most of it is MS-DIAL's; where MIL-X has a word
...
MS-DIAL writes the confidence of an annotation into the name itself, and MIL-X reads it back
...
of MIL-X's, or — for the peaks — back into the alignment result itself. See
[[projects-and-files]].

## Projects and results

An **MIL-X project** (`.milx`) is a small JSON file: the batch, the method text, the results
...
- The survey-scan cache and the MIL-X project file.
