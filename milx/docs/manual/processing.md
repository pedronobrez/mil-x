...
project file, or `milx_results` beside the first raw file for a batch that has no project.
...
| Stage | What happens |
| --- | --- |
| **Setup** | the output folder is created; the method is written to it as `milx_method.txt`; the retention-time correction step is prepared |
...
| **Done** | the result is loaded into Analytics and Statistics, and the MIL-X project is saved if it has a path |
