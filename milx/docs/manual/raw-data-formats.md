...
MS-DIAL reads raw data through a closed, Windows-only library. MIL-X replaces it with an open
...
Read by MIL-X's own streaming reader: zlib and MS-Numpress compression, 32- and 64-bit floats
...
- **Profile spectra are centroided by SCIEX's own peak finder**, which fits the apex and reports it about a quarter higher than a local maximum would. Peak heights feed the minimum-height cut-off, so this is the difference between a height ratio of 0.85 and one of 1.000 against MS-DIAL. `MILX_WIFF_CENTROID` selects `sciex` (default), `msdial` (the local-maximum method) or `0` (keep profile, and set the method to Profile).
...
`.wiff2` reader needs a native SQLite library that does not exist for this platform, so MIL-X
...
subfolder) and from `$MILX_PLUGINS`. Lower priority numbers win when two can read a file. A
