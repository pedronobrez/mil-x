...
| Desktop GUI | Avalonia app in `src/MilX.Desktop` (Explorer / Analytics / Method / Samples, OpenQuant's look) | same | the WPF GUI is not portable |
...
```bash
bash milx/scripts/setup-macos.sh          # .NET 8 SDK into ~/.dotnet (no sudo), NuGet source
bash milx/scripts/build-cli.sh            # -> milx/dist/milx-cli-osx-arm64/
milx/dist/milx-cli-osx-arm64/milx-cli lcms -i /data/run1 -o /data/run1/out -m method.txt -p
```
...
`.mdproject` that the Windows GUI (and the MIL-X desktop app) can open.
...
dotnet build MsdialWorkbench-MSDIAL-v5.5.260817/tests/MSDIAL5/MsdialCoreTestApp/MsdialCoreTestApp.csproj \
  -c Release -f net8 -p:UseOpenRawData=true -p:SkipLibraryDownload=true -o milx/build/cli-open
dotnet milx/build/cli-open/MSDIALCUI.dll lcms --help
bash milx/scripts/test.sh                 # unit tests + synthetic end-to-end run
```

## 2. Vendor formats on macOS/Linux

The vendor SDKs (Thermo, Agilent MHDAC, Bruker Baf2Sql/TDF, Sciex WiffReader, Waters, Shimadzu)
are Windows-only binaries, so MIL-X converts vendor files to mzML with ProteoWizard's
`msconvert` the first time they are read and then reuses `<name>.mzML` next to the raw file
(or in `$MILX_MZML_CACHE`). Nothing changes in your workflow: point `-i` at the folder with
...
1. **Docker (recommended on macOS).** Install Docker Desktop (enable Rosetta/x86-64 emulation on
   ...
   MIL-X runs `docker run --platform linux/amd64 ... wine msconvert <file> --mzML --64 --zlib
...
2. **Native msconvert.** If you have a Windows box or a Linux box with the pwiz Docker image,
   convert there and copy the `.mzML` files; or set `MILX_MSCONVERT=/path/to/msconvert`.
...
Environment variables: `MILX_MSCONVERT`, `MILX_DOCKER`, `MILX_PWIZ_IMAGE`,
`MILX_DOCKER_PLATFORM`, `MILX_MZML_CACHE`, `MILX_MSCONVERT_ARGS`,
`MILX_MSCONVERT_PROFILE=1` (keep profile data), `MILX_MSCONVERT_FORCE=1` (ignore cache).
...
Run `bash milx/scripts/fetch-sciex-assemblies.sh` once (it copies the Clearcore2 SDK from
the `alpharaw` Python package into `milx/vendor/sciex`, together with SCIEX's redistribution
...
OpenQuant reads them. Options: `MILX_WIFF_SAMPLE` (index or name inside a multi-sample
batch), `MILX_WIFF_CENTROID=0` (keep profile data; then set "MS1/MS2 data type: Profile"),
`MILX_WIFF_MIN_INTENSITY`. Verified on TripleTOF 5600 acquisitions through the whole
...
`<app>/plugins/legacy/`. MIL-X loads it by reflection for those extensions only. `.ibf` is pure
...
It lives in `~/Library/Caches/MIL-X/ms1` on macOS, `$XDG_CACHE_HOME/MIL-X/ms1` on Linux and
under `LocalAppData` on Windows; `MILX_CACHE` moves it. Preferences shows its size and empties
