...
## Decision
`MilX.RawData` treats vendor files as "mzML that has not been produced yet": on first
...
* Profile-mode workflows must set `MILX_MSCONVERT_PROFILE=1` (and MS-DIAL's data type).
