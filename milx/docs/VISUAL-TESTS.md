...
`tests/MilX.Desktop.Tests/VisualRegressionTests.cs` render a workspace with Skia, capture the
frame, and compare it with a stored one under `Baselines/`.

```bash
dotnet test milx/tests/MilX.Desktop.Tests/MilX.Desktop.Tests.csproj -c Release -p:UseOpenRawData=true
```
...
baseline that describes MIL-X and one that describes this laptop.
...
```bash
MILX_UPDATE_BASELINES=1 dotnet test milx/tests/MilX.Desktop.Tests/MilX.Desktop.Tests.csproj -c Release -p:UseOpenRawData=true
```
...
milx/scripts/smoke-ui.py --process ~/…/raw-folder --method method.txt --library library.msp
```

Both launch `/Applications/MIL-X.app` through LaunchServices, the way a person does, with an
...
alignment table and the MS-DIAL project were written, and that the MIL-X project now points at
...
`MILX_UI_PROBE` names a file, the window writes a small JSON snapshot to it whenever what it is
...
```bash
milx/scripts/ui-drive.sh front
milx/scripts/ui-drive.sh click 372 108     # the Statistics workspace tab
milx/scripts/ui-drive.sh key 23 cmd        # Cmd+5, the same thing by keyboard
milx/scripts/ui-drive.sh shot /tmp/now.png
```
