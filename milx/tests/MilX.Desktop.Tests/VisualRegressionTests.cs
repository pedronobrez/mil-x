...
/// MILX_UPDATE_BASELINES=1 to write the current frames as the new reference, and look at the
...
public class VisualRegressionTests
{
    ...
    private static string SourceBaselineDirectory
    {
        get
        {
            ...
            while (dir is not null && !File.Exists(Path.Combine(dir.FullName, "MilX.Desktop.Tests.csproj")))
            {
                dir = dir.Parent;
            }
    ...
    private static bool Updating => Environment.GetEnvironmentVariable("MILX_UPDATE_BASELINES") == "1";
    ...
    {
        var chart = new MilX.Desktop.Controls.SpectrumChart
        {
            Peaks = new[]
            {
        ...
        window.Show();
        window.Prepare(chart, "Representative MS/MS", new MilX.Desktop.Charts.ChartExportOptions());
