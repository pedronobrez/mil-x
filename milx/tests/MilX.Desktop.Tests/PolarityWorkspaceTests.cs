...
public class PolarityWorkspaceTests
{
    ...
    {
        var folder = Path.Combine(Path.GetTempPath(), "milx-polarity-ui-" + Guid.NewGuid().ToString("N"));
    ...
    {
        var folder = Path.Combine(Path.GetTempPath(), "milx-merge-ui-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(folder);
        try
        {
            var vm = new AnalyticsViewModel(new RawDataCache());
...
public class PolarityCurationTests
{
    ...
    {
        var folder = Path.Combine(Path.GetTempPath(), "milx-polarity-cur-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(folder);
        var vm = new AnalyticsViewModel(new RawDataCache());
