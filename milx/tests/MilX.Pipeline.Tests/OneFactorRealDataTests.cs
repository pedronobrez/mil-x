...
/// MILX_TEST_REVIEWED_ALIGNMENT points at that .arf2, and only reads it.
/// </summary>
public class OneFactorRealDataTests
{
    ...
    private static string? Source =>
        Environment.GetEnvironmentVariable("MILX_TEST_REVIEWED_ALIGNMENT") is { Length: > 0 } p && File.Exists(p) ? p : null;
    ...
    public void Confirmed_analytes_as_ratios_to_their_standards_go_through_the_whole_analysis()
    {
        ...
        if (source is null)
        {
            Console.WriteLine("skipped: set MILX_TEST_REVIEWED_ALIGNMENT to a reviewed alignment .arf2");
            return;
        }
