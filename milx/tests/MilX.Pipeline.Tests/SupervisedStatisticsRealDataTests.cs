...
/// Runs only when MILX_TEST_ALIGNMENT points at an alignment .arf2, and only reads it.
///
...
public class SupervisedStatisticsRealDataTests
{
    ...
    private static string? Source =>
        Environment.GetEnvironmentVariable("MILX_TEST_ALIGNMENT") is { Length: > 0 } p && File.Exists(p) ? p : null;
    ...
    public void A_real_alignment_refuses_labels_that_mean_nothing_and_takes_a_drift_correction()
    {
        ...
        if (source is null)
        {
            Console.WriteLine("skipped: set MILX_TEST_ALIGNMENT to an alignment .arf2");
            return;
        }
