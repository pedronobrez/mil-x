...
internal static class ClearcoreReader
{
    ...
    public static RawMeasurement Read(string path, int fileId, RawReadOptions options) {
        ...
        var centroidMode = (Environment.GetEnvironmentVariable("MILX_WIFF_CENTROID") ?? "sciex").Trim().ToLowerInvariant();
        ...
        var minIntensity = double.TryParse(Environment.GetEnvironmentVariable("MILX_WIFF_MIN_INTENSITY"), NumberStyles.Float, CultureInfo.InvariantCulture, out var mi) ? mi : 0.0;
