...
namespace MilX.Plugins.SciexWiff;
...
///   MILX_WIFF_SAMPLE     sample index (0-based) or sample name inside a multi-sample .wiff (default 0)
///   MILX_WIFF_CENTROID   1 (default) centroid profile spectra (MS-DIAL's local-maximum method),
///                            0 keep profile data (set "MS1/MS2 data type: Profile" in the method)
///   MILX_WIFF_MIN_INTENSITY  drop centroided peaks below this intensity (default 0)
/// </summary>
public sealed class SciexWiffReaderPlugin : IRawFileReaderPlugin
{
    ...
    internal static int ResolveSampleIndex(string path, IReadOnlyList<string> names) {
        ...
        var env = Environment.GetEnvironmentVariable("MILX_WIFF_SAMPLE");
