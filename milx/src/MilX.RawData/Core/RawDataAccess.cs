...
public class RawDataAccess : IDisposable
{
                ...
                case RawDataExtension.imzml: {
                    ...
                    if (legacy == null) {
                        RawDataAccessOptions.Log(
                            $"Format {_extensionString} is not implemented by MilX.RawData. Place the original RawDataHandler dll in <app>/plugins/legacy (or set MILX_LEGACY_RAWDATA_DLL) to read it, or convert the file to mzML.");
                        return null;
                    }
    ...
    public void ConvertToIABF(string outputDir = "") {
        throw new NotSupportedException("IABF writing is not implemented in MilX.RawData; use the original RawDataConverter on Windows.");
    }
