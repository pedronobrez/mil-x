...
namespace MilX.Pipeline.Statistics.Pathways;
...
public sealed record LipidReaction(string Id, string Reactant, string Product, ReactionKind Kind, IReadOnlyList<string> Genes, string Note, string Source)
{
    ...
    /// <summary>Transcribed from BioPAN's database, as opposed to a step MIL-X adds.</summary>
...
/// application, plus the steps MIL-X adds marked as such — and the map from MS-DIAL's ontology
...
public static class ReactionDatabase
{
    ...
    {
        using var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("MilX.Pipeline.Statistics.Pathways.reactions.tsv")
