...
#
#   scripts/carry-sciex.sh <application folder>    # the folder holding OpenDIAL / OpenDIAL.exe
#
...
ROOT="$(cd "$HERE/.." && pwd)"
SRC="${OPENDIAL_SCIEX_DIR:-$ROOT/vendor/sciex}"
...
for f in "$DEST"/*.dll; do
  name="$(basename "$f")"
  case "$name" in
    OpenDIAL.*|System.*|Microsoft.*|protobuf-net.dll) continue ;;
  esac
