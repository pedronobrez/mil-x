...
namespace OpenDIAL.RawData.Legacy;
...
/// The dll is loaded by reflection so that OpenDIAL never has a compile-time dependency
/// on it. It is used for formats OpenDIAL does not implement natively (ABF, IBF, NetCDF,
/// imzML) and, on Windows, for vendor formats when the vendor-enabled build is present.
///
/// Search order: $OPENDIAL_LEGACY_RAWDATA_DLL, then &lt;app&gt;/plugins/legacy/*.dll,
...
public sealed class LegacyReaderPlugin
{
    ...
    public static IEnumerable<string> CandidatePaths() {
        var env = Environment.GetEnvironmentVariable("OPENDIAL_LEGACY_RAWDATA_DLL");
