...
namespace OpenDIAL.RawData.Vendor;
...
{
    /// <summary>Path to a native msconvert executable (env OPENDIAL_MSCONVERT). "docker" forces the Docker path.</summary>
    public string? MsconvertPath { get; set; } = Environment.GetEnvironmentVariable("OPENDIAL_MSCONVERT");

    /// <summary>Docker executable (env OPENDIAL_DOCKER, default "docker").</summary>
    public string DockerPath { get; set; } = Environment.GetEnvironmentVariable("OPENDIAL_DOCKER") ?? "docker";

    /// <summary>ProteoWizard image (env OPENDIAL_PWIZ_IMAGE). The default image bundles the vendor readers under Wine.</summary>
    public string DockerImage { get; set; } = Environment.GetEnvironmentVariable("OPENDIAL_PWIZ_IMAGE") ?? "proteowizard/pwiz-skyline-i-agree-to-the-vendor-licenses:latest";

    /// <summary>Docker platform flag; the pwiz image is x86-64 only, so Apple Silicon needs emulation (env OPENDIAL_DOCKER_PLATFORM).</summary>
    public string DockerPlatform { get; set; } = Environment.GetEnvironmentVariable("OPENDIAL_DOCKER_PLATFORM") ?? "linux/amd64";

    /// <summary>Directory for converted mzML files (env OPENDIAL_MZML_CACHE). Null = next to the source file.</summary>
    public string? CacheDirectory { get; set; } = Environment.GetEnvironmentVariable("OPENDIAL_MZML_CACHE");

    /// <summary>Extra msconvert arguments (env OPENDIAL_MSCONVERT_ARGS), appended after the defaults.</summary>
    public string? ExtraArguments { get; set; } = Environment.GetEnvironmentVariable("OPENDIAL_MSCONVERT_ARGS");

    /// <summary>Apply vendor peak picking (centroiding) on all MS levels. MS-DIAL expects centroid data by default.</summary>
    public bool VendorPeakPicking { get; set; } = !string.Equals(Environment.GetEnvironmentVariable("OPENDIAL_MSCONVERT_PROFILE"), "1", StringComparison.Ordinal);

    /// <summary>Re-convert even when a cached mzML exists.</summary>
    public bool ForceReconvert { get; set; } = string.Equals(Environment.GetEnvironmentVariable("OPENDIAL_MSCONVERT_FORCE"), "1", StringComparison.Ordinal);
