...
public class UiProbeTests
{
    ...
    {
        var was = Environment.GetEnvironmentVariable("OPENDIAL_UI_PROBE");
        try
        {
            Environment.SetEnvironmentVariable("OPENDIAL_UI_PROBE", null);
            Assert.Null(UiProbe.FromEnvironment());
        }
        finally
        {
            Environment.SetEnvironmentVariable("OPENDIAL_UI_PROBE", was);
        }
    }
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "opendial-probe-" + Guid.NewGuid().ToString("N") + ".json");
        var was = Environment.GetEnvironmentVariable("OPENDIAL_UI_PROBE");
        try
        {
            Environment.SetEnvironmentVariable("OPENDIAL_UI_PROBE", path);
            ...
            Assert.Contains("OpenDIAL", root.GetProperty("title").GetString());
