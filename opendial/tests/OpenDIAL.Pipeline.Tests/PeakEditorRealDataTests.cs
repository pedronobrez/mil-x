...
/// back and read it again. Runs only when OPENDIAL_TEST_ALIGNMENT points at an alignment .arf,
/// and always on a copy, so a run is never edited by the test suite.
/// </summary>
public class PeakEditorRealDataTests
{
    private static string? Source => Environment.GetEnvironmentVariable("OPENDIAL_TEST_ALIGNMENT") is { Length: > 0 } p && File.Exists(p) ? p : null;
    ...
    public void Reintegrates_splits_saves_and_reloads_a_real_alignment()
    {
        ...
        if (source is null)
        {
            Console.WriteLine("skipped: set OPENDIAL_TEST_ALIGNMENT to an alignment .arf");
            return;
        }

        var work = Path.Combine(Path.GetTempPath(), "opendial-align-" + Guid.NewGuid().ToString("N"));
