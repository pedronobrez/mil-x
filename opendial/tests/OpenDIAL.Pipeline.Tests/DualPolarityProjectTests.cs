...
public class DualPolarityProjectTests
{
    ...
    {
        var folder = Path.Combine(Path.GetTempPath(), "opendial-dual-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(folder);
        try
        {
            var project = new OpenDialProject
            {
                Name = "liver",
            ...
            var read = await OpenDialProject.LoadAsync(path);
    ...
    {
        var folder = Path.Combine(Path.GetTempPath(), "opendial-dual-old-" + Guid.NewGuid().ToString("N"));
        Directory.CreateDirectory(folder);
        try
        {
