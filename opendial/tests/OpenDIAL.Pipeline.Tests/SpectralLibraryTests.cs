...
/// OPENDIAL_TEST_MSP points at a real one when there is time to read it.
/// </summary>
public class SpectralLibraryTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), "opendial-msp-" + Guid.NewGuid().ToString("N") + ".msp");
    ...
    {
        var path = Environment.GetEnvironmentVariable("OPENDIAL_TEST_MSP");
        if (path is null || !File.Exists(path)) { _out.WriteLine("skipped: set OPENDIAL_TEST_MSP"); return; }
