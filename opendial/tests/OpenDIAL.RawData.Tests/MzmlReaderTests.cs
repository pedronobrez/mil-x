...
using OpenDIAL.RawData.Tests.TestSupport;
using Xunit;

namespace OpenDIAL.RawData.Tests;

public class MzmlReaderTests : IDisposable
{
    private readonly string _dir = Path.Combine(Path.GetTempPath(), "opendial-tests-" + Guid.NewGuid().ToString("N"));
    ...
    public void Precursor_fields_and_collision_energy_are_populated() {
        ...
        // OpenDIAL deviation: activation CE is surfaced on the spectrum and in the CE target list
