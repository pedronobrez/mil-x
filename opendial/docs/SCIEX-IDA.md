...
validation data uses, and three details of it decide whether OpenDIAL reproduces a Windows
...
percentile 0.997–1.004). `OPENDIAL_WIFF_CENTROID` selects the behaviour: `sciex` (default),
...
Running OpenDIAL with `Acquisition type: DDA` on the same data assigns MS/MS to 897 peaks; with
`SWATH`, to 1 218, against MS-DIAL's 1 206.

## Where this lands

Eight ZenoTOF 7600 injections, MS-DIAL 5.5 on Windows against OpenDIAL on macOS with the parameter
