# MS-DIAL 5 reverse-engineering report and OpenDIAL architecture
...
what the closed components do, and how OpenDIAL rebuilds the missing pieces so the software runs
...
NCDK <- CommonStandard <- MsdialCore <- {LcMs, GcMs, Dims, Imms, LcImMs, ImmsImaging} <- MsdialIntegrate <- MSDIALCUI
                            ...
                            ^-- RawDataHandler (closed NuGet)  ==> replaced by OpenDIAL.RawData
```
...
Behaviour that downstream code relies on and that OpenDIAL reproduces exactly:
...
1. **The last peak of every spectrum is dropped** (both zlib and uncompressed arrays): a 195-peak
   spectrum comes back with 194 peaks and `HighestObservedMz` is wrong. OpenDIAL keeps all peaks;
...
threshold in the method file was misread, silently producing zero peaks. OpenDIAL forces the
...
```
opendial/
  src/OpenDIAL.RawData/        open reader, drop-in for CompMs.RawDataHandler.Core (netstandard2.0; net8.0)
...
RawDataAccess.GetMeasurement()
  ...
  │        $OPENDIAL_MZML_CACHE), produced by msconvert (native exe on PATH / $OPENDIAL_MSCONVERT,
...
* `System.Data.SQLite` has no osx-arm64 native library; OpenDIAL builds compile the EAD lipid
