...
```bash
bash opendial/scripts/setup-macos.sh          # o SDK do .NET 8 em ~/.dotnet, sem sudo
bash opendial/scripts/fetch-sciex-assemblies.sh   # opcional: o SDK da SCIEX, para .wiff nativo
...
Silicon precisa para o correr; não é notarizado. A versão vem de `opendial/Directory.Build.props`
...
redistribui sob a licença do WIFF Reader Distributable SDK da SCIEX, para `opendial/vendor/sciex`
...
| Suíte | O que contém |
| --- | --- |
| `OpenDIAL.RawData.Tests` | o leitor de mzML: codificações, idas e voltas de Numpress, a semântica de que o código a jusante depende, a ponte para fabricantes |
| `OpenDIAL.Pipeline.Tests` | o depósito de curadoria, o editor de picos, a busca na biblioteca, os caches, a estatística (com testes de equivalência entre a validação cruzada simples e a rápida), a corrida sintética ponta a ponta, e testes com dados reais que correm quando `OPENDIAL_TEST_ALIGNMENT` aponta para um resultado real |
| `OpenDIAL.Desktop.Tests` | as vistas reais, sem tela: a área de revisão, a área de estatística, os gestos, as fontes, a sonda, o manual nos dois idiomas, e os quadros de regressão visual |
| `OpenDIAL.Interop.OpenQuant.Tests` | o CSV de componentes e a importação de lote |
| `OpenDIAL.Plugins.SciexWiff.Tests` | o leitor nativo, sobre um `.wiff` real quando `OPENDIAL_TEST_WIFF` nomeia um |
...
sob `tests/OpenDIAL.Desktop.Tests/Baselines/`. Os quadros viajam entre máquinas porque a aplicação
...
Lança `/Applications/OpenDIAL.app` pelo LaunchServices com uma pasta de configurações isolada, e lê
...
```bash
python3 opendial/scripts/build-manual.py     # -> opendial/dist/OpenDIAL-manual-<version>.pdf e .html
python3 opendial/scripts/build-manual.py --lang pt   # -> opendial/dist/OpenDIAL-manual-pt-<version>.pdf e .html
```
