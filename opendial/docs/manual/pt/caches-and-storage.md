...
Vive em `~/Library/Caches/OpenDIAL/spectra` no macOS, `$XDG_CACHE_HOME/OpenDIAL/spectra` no Linux
e sob `LocalAppData\OpenDIAL\spectra` no Windows; `OPENDIAL_CACHE` move-o. [[settings]] mostra o
...
da pasta de cache de conversão definida em [[settings]] (`OPENDIAL_MZML_CACHE` faz o mesmo). É
reutilizado enquanto for mais novo do que a origem; `OPENDIAL_MSCONVERT_FORCE=1` reconverte. Esses
...
`settings.json` na pasta de dados da aplicação — `~/.config/OpenDIAL` no macOS e Linux,
`%APPDATA%\OpenDIAL` no Windows, ou onde `OPENDIAL_SETTINGS_DIR` apontar. O que guarda está em
