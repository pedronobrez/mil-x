...
section: Reference
order: 44
summary: O console opendial-cli para processar sem a janela, os scripts de smoke e de comando, e as ferramentas de diagnóstico.
---
...
O console do próprio MS-DIAL (`MSDIALCUI`), construído com a camada de dados brutos do OpenDIAL e
publicado como binário autocontido por `scripts/build-cli.sh` em `dist/opendial-cli-<rid>/`.
Corre o mesmo motor que a aplicação de desktop, sem a janela, que é o que um servidor ou um script
de lote quer.

```bash
opendial-cli lcms -i /data/run1 -o /data/run1/out -m method.txt -p
```
...
O lançador `opendial-cli` força a cultura invariante antes de começar; use-o em vez de
...
Todos sob `opendial/scripts/`; veja [[building-and-testing]] para os de construção.
...
| `make-app-bundle.sh` | publica e empacota `dist/OpenDIAL.app` |
...
Sob `opendial/tools/`, corridas com `dotnet run --project tools/<name> -- …`:
