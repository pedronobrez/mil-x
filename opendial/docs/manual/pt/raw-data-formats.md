...
O MS-DIAL lê dados brutos por uma biblioteca fechada, só para Windows. O OpenDIAL substitui-a por
...
Lido pelo leitor de streaming do próprio OpenDIAL: compressão zlib e MS-Numpress, floats e
...
- **Os espectros de perfil são centroidados pelo detector de picos da própria SCIEX**, que ajusta o ápice e reporta-o cerca de um quarto mais alto do que um máximo local faria. As alturas de pico alimentam o corte de altura mínima, de modo que esta é a diferença entre uma razão de alturas de 0,85 e uma de 1,000 contra o MS-DIAL. `OPENDIAL_WIFF_CENTROID` escolhe `sciex` (padrão), `msdial` (o método do máximo local) ou `0` (manter perfil, e pôr o método em Profile).
...
plataforma, de modo que o OpenDIAL lê um `.wiff2` pelo `.wiff` ao lado: os mesmos espectros, os
...
1. **Docker** — o Docker Desktop com a imagem oficial `proteowizard/pwiz-skyline-i-agree-to-the-vendor-licenses`, que traz os leitores dos fabricantes sob Wine; em Apple Silicon corre sob emulação x86-64, lentamente (minutos por arquivo) mas exatamente. Faça `docker pull` da imagem uma vez.
...
(qualquer subpasta) e de `$OPENDIAL_PLUGINS`. Números de prioridade mais baixos vencem quando dois
