...
| **Beyond BioPAN** | também os passos que o OpenDIAL acrescenta à rede para classes que uma corrida de lipidômica confirma e o BioPAN não cobre — Cer ↔ HexCer ↔ LacCer, HexCer ↔ SHexCer, Chol ↔ CE, FA ↔ CAR, MG → FA, PE → PA — marcados `extension` na tabela; desligado por padrão, de modo que o resultado padrão é a rede do BioPAN e nada mais |
