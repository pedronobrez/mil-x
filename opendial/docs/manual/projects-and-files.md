...
order: 30
summary: The OpenDIAL project file, the MS-DIAL project, every file in a results folder and what each holds, the curation files, backups, and how paths are kept.
---

# Projects and files

## The OpenDIAL project (.odproj)
...
MS-DIAL on Windows opens it; OpenDIAL opens it with **Open project…** or from Finder, and rebuilds
the batch from it when there is no `.odproj`. It is the one file that carries the library inside
...
| `AlignResult-<stamp>_curation.json` | OpenDIAL's sidecar: comments, hand-picked names, reviewed flags |
...
`opendial_method.txt` when it exists — enough to review, not enough to rerun without checking the
batch first.
