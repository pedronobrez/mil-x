...
- The species level follows BioPAN's rule for the chain reactions: a step that releases a chain is drawn only when that fatty acid is measured, one that adds a chain only when its acyl-CoA is.
