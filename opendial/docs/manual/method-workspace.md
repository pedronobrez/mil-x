...
written to the results folder as `opendial_method.txt` at the start of every run, which is how a
...
writes it back. Opening a results folder with no project reads `opendial_method.txt` from it
