...
included. `OPENDIAL_THEME=dark` or `light` forces one for a session without touching the setting.
...
`settings.json` under `~/.config/OpenDIAL` on macOS and Linux, `%APPDATA%\OpenDIAL` on Windows,
or the folder `OPENDIAL_SETTINGS_DIR` names. Besides the three sections above it remembers what the
