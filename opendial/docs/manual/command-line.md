...
section: Reference
order: 44
summary: The opendial-cli console for processing without the window, the smoke and drive scripts, and the diagnostic tools.
---
...
MS-DIAL's own console (`MSDIALCUI`), built with OpenDIAL's raw-data layer and published as a
self-contained binary by `scripts/build-cli.sh` into `dist/opendial-cli-<rid>/`. It runs the same
...
The `opendial-cli` launcher forces the invariant culture before starting; use it rather than
...
All under `opendial/scripts/`; see [[building-and-testing]] for the build ones.
...
Under `opendial/tools/`, run with `dotnet run --project tools/<name> -- …`:
