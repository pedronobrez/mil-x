...
license. OpenDIAL is itself a .NET application, so it can link to them directly instead of
going through Python.

## Decision
`OpenDIAL.Plugins.SciexWiff` implements `IRawFileReaderPlugin` on top of Clearcore2 and is
