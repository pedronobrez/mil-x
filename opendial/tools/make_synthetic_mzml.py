#!/usr/bin/env python3
"""
Synthetic LC-MS/MS (DDA) dataset generator for OpenDIAL / MS-DIAL testing.
...
def write_mzml(path, name, spectra, use_zlib):
    head = ('<?xml version="1.0" encoding="utf-8"?>\n'
            ...
            '<cvParam cvRef="MS" accession="MS:1000799" name="custom unreleased software tool" value="OpenDIAL synthetic generator"/>'
...
def write_gc_msp(path):
    with open(path, "w") as f:
        for cname, rt, base, frags in GC_COMPOUNDS:
            ...
            f.write("COMMENT: OpenDIAL synthetic EI reference\n")
...
def write_msp(path):
    with open(path, "w") as f:
        for cname, mass, formula, rt, base, frags in COMPOUNDS:
            ...
            f.write("COMMENT: OpenDIAL synthetic reference\n")
            f.write(f"Num Peaks: {len(frags)}\n")
            for fmz, rel in frags:
