#!/usr/bin/env python3
"""Converts an MS-DIAL parameter export into an OpenDIAL method file.

MS-DIAL writes "<Project>_param_<stamp>.txt" next to a processed dataset. Most of its keys are
the ones OpenDIAL's method parser already understands, so reproducing a Windows MS-DIAL run on
...
# Keys copied straight through, in the order OpenDIAL's method files use.
SECTIONS = [
    ("#Data type", [
